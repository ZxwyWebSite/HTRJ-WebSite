...
$pjht = 'static/api/beta/searchKeywordsList/testindex.jht'; //主模板路径
if (file_exists($pjht)) {
    $jht = file_get_contents($pjht); //载入
    include('static/api/beta/searchKeywordsList/' . $id . '/setting.php'); //其它设置
    ...
    for ($i = 0; $i < $pagesize; $i++) {
        $file = 'static/api/beta/searchKeywordsList/' . $id . '/' . ($i + $p) . '.js'; //文件
        if (file_exists($file)) {
